...
import { links as linksApi } from '../../servicos/api';
...
export default function LinkInicial() {
  ...
  const carregarLinks = async () => {
    try {
      const dados = await linksApi.listar();
      setLinks(dados);
    } catch (erro) {
  ...
  const tratarSalvar = async (dados) => {
    try {
      if (linkAtivo) {
        await linksApi.atualizar(linkAtivo.id, dados);
        toast.success('Link atualizado');
      } else {
        await linksApi.criar(dados);
        toast.success('Link adicionado');
      }
      setModalAddAberto(false);
      carregarLinks();
    } catch (erro) {
      toast.error(erro.message || 'Erro ao salvar link.');
    }
  };

  const tratarDeletar = async () => {
    try {
      await linksApi.deletar(linkAtivo.id);
      toast.success('Link deletado com sucesso');
      setModalDeletarAberto(false);
      carregarLinks();
    } catch (erro) {
      toast.error(erro.message || 'Erro ao deletar link.');
    }
  };

  // agrupa por categoria
  const categorias = links.reduce((acc, link) => {
    const cat = link.categoria || 'Sem categoria';
    if (!acc[cat]) acc[cat] = [];
    acc[cat].push(link);
    return acc;
  }, {});
  ...
  return (
    <div className="pagina-inicial-container">
      ...
      <ModalObservacao
        ...
        aoSalvar={(obs) => {
          // >>> BACKEND: adicionar campo "observacao" na tabela links e endpoint PATCH /api/links/:id/observacao <<<
